...
| 월 누적 비용 추이 | 시간대별 비용 변화 (전체 서비스 합계 기준) |
| 일별 누적 비용 현황 | 일별 시점의 누적 비용을 막대로 비교 (전날보다 줄지 않는 게 정상이며, 그날 하루 지출액이 아닌 누적 스냅샷임) |
| 서비스별 비용 비율 | EC2, RDS, ALB, VPC, ElastiCache, SecretsManager, CloudWatch 서비스별 비용 비율 파이 차트 |
